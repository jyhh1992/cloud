...
# DB 생성
conn = sqlite3.connect("wordrecord.db", isolation_level=None)
cur = conn.cursor()
...
# db 테이블 증가
cur.execute("create table if not exists usersdb(id integer primary key autoincrement, \
            corr_cnt text not null, exe_time text not null, regdate text not null)")
...
cur.execute("INSERT INTO usersdb ( corr_cnt, exe_time, regdate) VALUES (?,?,?)", (corr_cnt, exe_time, nowDatetime))
conn.commit()
